...
struct _GggProviderDialogPrivate {
  GtkListStore *store;
  GtkWidget *combo;
};

#define GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), GGG_TYPE_PROVIDER_DIALOG, GggProviderDialogPrivate))
G_DEFINE_TYPE (GggProviderDialog, ggg_provider_dialog, GTK_TYPE_DIALOG);

static void
populate_store (GtkListStore *store, RestXmlNode *c_node)
{
...
static void
ggg_provider_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
  GggProviderDialog *dialog = GGG_PROVIDER_DIALOG (object);

  switch (property_id) {
  case PROP_COUNTRY:
    {
      RestXmlNode *country_node;

      country_node = g_value_get_boxed (value);
      populate_store (dialog->priv->store, country_node);
...
static void
ggg_provider_dialog_class_init (GggProviderDialogClass *klass)
{
  GObjectClass *o_class = G_OBJECT_CLASS (klass);
  GParamSpec *pspec;

  g_type_class_add_private (klass, sizeof (GggProviderDialogPrivate));

  o_class->set_property = ggg_provider_set_property;

  pspec = g_param_spec_boxed ("country", "country", "country",
                              REST_TYPE_XML_NODE,
                              G_PARAM_STATIC_STRINGS | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY);
  g_object_class_install_property (o_class, PROP_COUNTRY, pspec);
}

static void
ggg_provider_dialog_init (GggProviderDialog *self)
{
  ...
  label = gtk_label_new (_("Select Provider"));
...
RestXmlNode *
ggg_provider_dialog_get_selected (GggProviderDialog *dialog)
{
  GtkComboBox *combo;
  GtkTreeIter iter;
